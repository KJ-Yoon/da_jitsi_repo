...
import * as fnRecorde from './recorder.js';
import UI from '../../../../../modules/UI/UI';
import { ModeratorIndicator } from '../../../filmstrip/components/web';
...
class Toolbox extends Component<Props, State> {
    ...
    constructor(props: Props) {
        ...
        // 이벤트 핸들러를 바닌드 하여 인스턴스 당 한 번만 바인드
        ...
        this._onToolbarToggleDaRecording = this._onToolbarToggleDaRecording.bind(this);
     ...
     * ToolbarButtons 작업을 트리거하기 위한 키보드 단축키를 설정
     * @inheritdoc
     * @returns {void}
     */
    componentDidMount() {
        ...
        //setTimeout(alert("짜잔"), 70000);
        //setTimeout(function(){ alert("Set"); }, 3000);
    ...
    render() {
     ...
     componentDidMount() {
       ...
       // this.props.dispatch(setPreferredVideoQuality(LOW));
     }
    ...
    _doToggleRaiseHand() {
        ...
        this.props.dispatch(participantUpdated({
            ...
            // 로컬 참가자는 로컬 참가자가 하나만 있다는 사실에 의해 고유하게 식별되므로,
            // 로컬 참가자 만 JitsiConference 인스턴스(즉, 원격 참가자의 참가자 속성 '회의')를 
            // 지정하지 않고 업데이트 할 수 있습니다.
    ...
    _doToggleDaRecording() {
        const { _localParticipantID, _daRecording } = this.props;

        this.props.dispatch(daParticipantUpdated({ 
            id: _localParticipantID,
            local: true,
            daRecording: !_daRecording
        }));
    }
    ...
    _doToggleVideoQuality() {
        //단축키로 품질로 A를 눌러야 볼 수있다.
        alert("Toobax.js - toggleDialog");
    ...
    */
    ...
    _onToolbarOpenVideoQuality() {
        ...
        //alert("Toolbox.js - _onToolbarOpenVideoQuality");
    ...
    _onToolbarToggleRaiseHand() {
        sendAnalytics(createToolbarEvent(
            'raise.hand', //의미 x
            { enable: !this.props._raisedHand })); //글자 토글에 대해서는 의미 x

        this._doToggleRaiseHand(); //의미 o
    }

    //★★ DaRecording
    _onToolbarToggleDaRecording: () => void;

    async _onToolbarToggleDaRecording() {
        let moderatorflag = fnRecorde.moderatorFlagSelect();
        
        if(moderatorflag){
            let flag = fnRecorde.flag();
        
            if(flag) {
                //console.log(flag);     
                let startSuccessFlag = await fnRecorde.start();
                if (startSuccessFlag){
                    this._doToggleDaRecording();
                    
                    fnRecorde.recordingStartCss();
                    
                    APP.UI.messageHandler.notify(
                        'dialog.daRecordingStartTitle'
                    );
                }else{
                    APP.UI.messageHandler.notify(
                        'dialog.daRecordingCancelTitle'
                    );
                }
                ...
                fnRecorde.recordingStopCss();
                
                APP.UI.messageHandler.notify(
                    'dialog.daRecordingStopTitle'
                );
            }
        }else{
            APP.UI.messageHandler.notify(
                'dialog.daRecordingNotUse'
            );
        }   
     ...
     * 프로파일 버튼이 표시되면 true, 그렇지 않으면 false를 반환합니다.
     *
     * @returns {boolean}
     */
    _isProfileVisible() {
    ...
    _renderOverflowMenuContent() {
        ...
        return [
            ...
            //★★ DaRecording
            this._shouldShowButton('daRecording')
                && <OverflowMenuItem
                    accessibilityLabel = { t('toolbar.daRecording') }
                    icon = { IconRec }
                    key = 'daRecording'
                    onClick = { this._onToolbarToggleDaRecording }
                    text = { _daRecording ? t('toolbar.daRecordingStop') : t('toolbar.daRecordingStart') } />,
            //★★   
...
function _mapStateToProps(state) {
    ...
    let isModerator = localParticipant.role === PARTICIPANT_ROLE.MODERATOR;
    fnRecorde.moderatorFlagInsert(isModerator);
    ...
    return {
        ...
        _daRecording: localParticipant.daRecording,
