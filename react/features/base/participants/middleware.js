 ...
 * CONFERENCE_JOINED 및 CONFERENCE_LEFT 조치를 취하고, 각각 로컬 참가자의 ID를 업데이트 하는 미들웨어
 *
        ...
        if (conference) {
            ...
            conference.on(
                ...
                (participant, propertyName, oldValue, newValue) => {
                    ...
                    //★
                    case 'daRecording': {
                        _daRecordingUpdated(store, conference, participant.getId(), newValue);
                        break;
                    }
                    default:
                        // Ignore for now.
            ...
            _daRecordingUpdated(store, conference, localParticipantId, false);
        }
    }
);
...
function _participantJoinedOrUpdated({ dispatch, getState }, next, action) {
    const { participant: { avatarURL, e2eeEnabled, email, id, local, name, raisedHand, daRecording } } = action;
    ...
    if (typeof daRecording !== 'undefined') {
        if (local) {
            const { conference } = getState()['features/base/conference'];
...
function _raiseHandUpdated({ dispatch, getState }, conference, participantId, newValue) {
    console.log("★_raiseHandUpdated function★");
...
//★
function _daRecordingUpdated({ dispatch, getState }, conference, participantId, newValue) {
    //console.log("★★ _daRecordingUpdated function ★★");
    const daRecording = newValue === 'true';
    //const raisedHand = newValue === 'true';
    ...
    if (daRecording) {      
        dispatch(showNotification({
            ...
            //문구
            titleKey: 'notify.daRecordingStart'
        }, NOTIFICATION_TIMEOUT))
    }else {
        //\접속이 끊킬 시 == 접속 종료에 나타나는 메시지로 주석처리
        // dispatch(showNotification({
        //     titleArguments: {
        //         name: getParticipantDisplayName(getState, participantId)
        //     },
        //     titleKey: 'notify.daRecordingStop'
        // }, NOTIFICATION_TIMEOUT))
    }
}
